...
const SQL_SELECT_ALL = 'SELECT * FROM X00158259.Achievement';
// without_array_wrapper - use for single result

const SQL_SELECT_BY_ID = 'SELECT * FROM X00158259.Achievement WHERE id = @id for json path, without_array_wrapper;';

// Second statement (Select...) returns inserted record identified by id = SCOPE_IDENTITY()
//need to look at changing next few lines
const SQL_INSERT = 'INSERT INTO X00158259.Achievement (achievementId, studentId, achievement, achievementDate, teacher, comment) VALUES (@achievementId, @studentId, @achievement, @achievementDate, @teacher, @comment); SELECT * from X00158259.Achievement WHERE id = SCOPE_IDENTITY();';


/**
 * GET a list of all or if search criteria is set filter
 * Address http://server:port/Achievement
...
/**
 * GET single by id
 * Address http://server:port/Achievement/:id
...
router.get('/:id', async (req, res) => {
     ...
     * See Achievement to validator npm package (at top) for docs
     * If validation fails return an error message
     */
    if (!validator.isNumeric(AchievementId, { no_symbols: true })) {
     ...
     * Single Achievement with matching id is returned
     */
    try {
        const pool = await dbConnPoolPromise
        const result = await pool.request()
