...
const SQL_SELECT_ALL = 'SELECT * FROM X00158259.Link';
// without_array_wrapper - use for single result

const SQL_SELECT_BY_ID = 'SELECT * FROM X00158259.Link WHERE id = @id for json path, without_array_wrapper;';

// Second statement (Select...) returns inserted record identified by id = SCOPE_IDENTITY()
//need to look at changing next few lines
const SQL_INSERT = 'INSERT INTO X00158259.Link (topicId, linkDate, linkName, description, notes, linkURL) VALUES (@topicId, @linkDate, @linkName, @description, @notes, @linkURL); SELECT * from X00158259.Link WHERE id = SCOPE_IDENTITY();';

const SQL_UPDATE = 'UPDATE X00158259.Link SET topicId = @topicId, linkDate = @linkDate, linkName = @linkName, description = @description, notes = @notes, linkURL = @linkURL WHERE id = @id; SELECT * FROM X00158259.Link WHERE id = @id;';

const SQL_DELETE = 'DELETE FROM X00158259.Link WHERE id = @id;';


/**
 * GET a list of all or if search criteria is set filter
 * Address http://server:port/link
...
router.get('/:id', async (req, res) => {
     ...
     * Single link with matching id is returned
     */
    try {
        const pool = await dbConnPoolPromise
        const result = await pool.request()
...
function validate(req, isUpdate) {
    ...
    const linkName = validator.escape(req.body.linkName);
    if (linkName === "") {
        errors += "invalid linkName; ";
    }
